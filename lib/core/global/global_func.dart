...
import '../component/global_widgets.dart';
import '../component/media_upload/upload_bloc/upload_bloc.dart';
...
String convertDateToStr(DateTime dateTime) {
  return "${DateFormat('d MMM yyyy', navigatorKey.currentState!.context.locale.languageCode).format(dateTime)} "
      "- ${DateFormat('d MMM yyyy', navigatorKey.currentState!.context.locale.languageCode).format(dateTime)}";
}

String getInitials(String? text) {
  if (text == null || text.trim().isEmpty) return "";

  // split by spaces and remove extra empty entries
  final parts = text.trim().split(RegExp(r"\s+"));

  if (parts.length == 1) {
    // Only one word → take first 2 chars if available
    return parts[0].substring(0, parts[0].length >= 2 ? 2 : 1).toUpperCase();
  }

  // Take first letter of first two words
  String first = parts[0].isNotEmpty ? parts[0][0] : "";
  String second = parts[1].isNotEmpty ? parts[1][0] : "";
